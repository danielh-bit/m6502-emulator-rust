...
impl Assembler {
    pub fn assemble() -> (Vec<u8>, u16) {
        let (mut lex_codes, offset) = Lexer::lex(read_to_string("program.txt").unwrap()).unwrap();
...
pub struct Lexer {}
impl Lexer {
    //returns the lex codes and the offset (origin of the program).
